...
void lookall(){
        ...
        for(int i;i < Stack_pointer;i++){
                switch (Symbol_Stack[i].type) {
                        case GLOBAL_VAR:
                                printf("== GLOBAL       %s\n", Symbol_Stack[i].vname);
                                break;
                        case LOCAL_VAR:
                                printf("== LOCAL    %d   %s\n", Symbol_Stack[i].val, Symbol_Stack[i].vname);
                                break;
                        case PROC_NAME:
                                printf("== PROC         %s\n", Symbol_Stack[i].vname);
                                break;
...
Factor insert(char *name, int flag) {
        ...
        strcpy(Symbol_Stack[Stack_pointer].vname, name);
        if (Symbol_Stack[Stack_pointer].type == LOCAL_VAR) {
                Symbol_Stack[Stack_pointer].val = Last_Register;
                Last_Register++;
        }
        tmp = Symbol_Stack[Stack_pointer];
        Stack_pointer++;
        lookall();
        return tmp;
}
...
void displayLlvmfundecl( Fundecl *decl ,FILE *fp){
        ...
        if(global_codehd != NULL){
            displayLlvmcodes( global_decl->codes,fp) ;
        }
        if( decl == NULL ) return;
...
expression 
        ...
        | expression PLUS term
        {
                ...
                retval.val = Last_Register; /* 新規のレジスタ番号を取得 */
        ...
        { 
                ...
                switch(Proc_Term){
                        case GLOBAL_VAR:
                                ...
                                (x->args).load.retval = tmp; /* 命令の第 2 引数を指定 */
                                add_globalnode(x);
                                break;
                        case LOCAL_VAR:
                                x = (LLVMcode *)malloc(sizeof(LLVMcode)); /*メモリ確保 */
